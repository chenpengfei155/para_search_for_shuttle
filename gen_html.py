...
async function deleteSelected() {
  const status = document.getElementById('copy-status');
  if (!deleteApiUrl) {
    status.textContent = 'delete is unavailable for this report';
    status.style.color = '#b00';
    setTimeout(() => { status.textContent = ''; status.style.color = '#060'; }, 2000);
    return;
  }
  if (selected.size === 0) {
    status.textContent = 'nothing selected';
  ...
  const keys = [...selected].sort((a, b) => a - b).map(i => recordKeyForApi(records[i]));
  if (!window.confirm(`Delete ${keys.length} selected record(s)?`)) return;
  ...
  try {
    ...
    status.textContent = `deleted ${result.removed} record(s), reloading...`;
    selected.clear();
    setTimeout(() => { window.location.reload(); }, 250);
  } catch (err) {
    status.textContent = `delete failed: ${err}`;
    status.style.color = '#b00';
    setTimeout(() => { status.textContent = ''; status.style.color = '#060'; }, 2500);
  }
}
